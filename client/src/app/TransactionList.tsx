...
const TransactionList = ({ transactions, onEdit, onDelete, isLoading }) => {
  ...
  const filteredTransactions = transactions?.filter((transaction) => {
    ...
    return (
      transaction.tenpistaName.toLowerCase().includes(searchLower) ||
      transaction.businessCategory.toLowerCase().includes(searchLower)
    );
  }) || [];
  ...
  const sortedTransactions = [...filteredTransactions].sort((a, b) => {
    ...
    switch (sortBy) {
      ...
      case 'amount':
        ...
        break;
      case 'name':
        aValue = a.tenpistaName.toLowerCase();
        bValue = b.tenpistaName.toLowerCase();
        break;
      default:
        return 0;
    }
            ...
            <tbody>
              {sortedTransactions.map((transaction) => (
                <tr key={transaction.id}>
                  <td className="name-cell">{transaction.tenpistaName}</td>
                  <td className="amount-cell">
                    {formatCurrency(transaction.amount)}
                  </td>
                  <td className="business-cell">{transaction.businessCategory}</td>
